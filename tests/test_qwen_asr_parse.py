...
def test_parse_observation_accepts_json_with_uncertain_spans():
    ...
    assert result["uncertainty"] == {"confidence": {}, "text_candidates": {}}


def test_parse_observation_keeps_same_language_english_candidates():
    raw = '{"text":"the project uses vibo","uncertain_spans":[{"span":"vibo","candidates":["vibo","vivo"],"confidence":0.2}]}'
    result = parse_observation(raw)
    assert result["uncertainty"]["confidence"]["vibo"] == 0.2
    assert result["uncertainty"]["text_candidates"]["vibo"] == ["vibo", "vivo"]
...
def test_parse_uncertainty_tags_against_fixed_text():
    ...
    assert tagged == {"confidence": {}, "text_candidates": {}}
...
def test_streaming_asr_enriches_uncertainty_before_publishing(monkeypatch, tmp_path):
    ...
    assert published[0][2]["text_candidates"] == {}
