...
class RealtimeAnalysisCoordinator:
    ...
    def submit(self, session_id: str, turn_id: str) -> Future[Any]:
        with self._lock:
            if self._closed:
                raise RuntimeError("realtime coordinator is closed")
            previous = self._tails.get(session_id)
            future = self.executor.submit(self._run_after, previous, session_id, turn_id)
            self._tails[session_id] = future
            return future

    def _run_after(self, previous: Future[Any] | None, session_id: str, turn_id: str) -> Any:
